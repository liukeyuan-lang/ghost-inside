...
func pulse() -> void:
	...
	if player.distance_to(targets[GameState.segment]) > 130:
		...
		notice_time = 3.0
		return
	var bundles := [["F02", "E03", "B02"], ["F03", "E02", "B03"], ["F01", "E01", "B01"]]
...
func _draw() -> void:
	...
	if GameState.segment == 0:
		draw_actor("lin_father", Vector2(420, 510), 285, Color(0.83, 0.86, 0.90))
		draw_actor("lin_che", Vector2(820, 510), 275, Color(0.70, 0.80, 0.88))
		...
		draw_actor("shen_zhou", Vector2(390, 510), 270, Color(0.78, 0.90, 0.86))
		draw_actor("lin_che", Vector2(820, 510), 280, Color(0.64, 0.72, 0.80))
		...
		draw_actor("lin_che", Vector2(430, 510), 278, Color(0.68, 0.78, 0.86))
		draw_actor("lin_yuan", Vector2(830, 510), 250, Color(0.86, 0.89, 0.94))
	...
	draw_actor("therapist", player + Vector2(0, 22), 92, CYAN if immunity <= 0 else Color("c2ffff"), facing_left, bob)
