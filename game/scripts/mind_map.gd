...
var carpet: Texture2D = preload("res://assets/map/office_carpet.png")
var office_terminal: Texture2D = preload("res://assets/map/office_terminal.png")
var fan_cage: Texture2D = preload("res://assets/map/fan_cage.png")
func _ready() -> void:
...
func interact(index: int) -> void:
	...
	var layer_texts: Variant = obj.get("frequency_text", [])
	var layer_text: String = str(layer_texts[frequency]) if layer_texts is Array and layer_texts.size() > frequency else str(obj.inspect_text)
...
func _draw() -> void:
	...
	draw_actor("林宇", Vector2(470, 330), 112, Color(0.62, 0.78, 0.86, 0.34))
	draw_actor("陈默", Vector2(535, 330), 112, Color(0.86, 0.58, 0.42, 0.30))
	draw_actor("林远", Vector2(690, 330), 118, Color(0.70, 0.82, 0.92, 0.30))
	draw_texture_rect(fan_cage, Rect2(924, 230, 84, 84), false, Color(0.35, 0.62, 0.68, 0.32))
	for i in range(3):
		...
		if i == 0:
			...
			draw_texture_rect(office_terminal, Rect2(points[i] - Vector2(48, 35), Vector2(96, 72)), false, Color(0.42, 0.68, 0.72))
	...
	draw_actor("怪物", Vector2(995, 366), 142, Color(0.55, 0.72, 0.78, 0.72) if GameState.applause_off else Color(0.95, 0.55, 0.60, 0.82))
	...
	draw_actor("心灵调理师", player + Vector2(0, 24), 94, Color(0.82, 0.96, 1.0), facing_left, bob)
