...
var _rejected_ports: Dictionary = {}  # port -> true (handshake rejected by server)
...
func _process(delta: float) -> void:
	...
	for p in range(BASE_PORT, MAX_PORT + 1):
		...
		# No peer - try reconnect on timer (skip rejected ports)
		if ws == null:
			if _rejected_ports.get(p, false):
				continue
		...
		match state:
			...
			WebSocketPeer.STATE_CLOSED:
				if _connected.get(p, false):
					...
					client_disconnected.emit()
				# Check for project-mismatch rejection (close code 1008)
				if ws and ws.get_close_code() == 1008:
					_rejected_ports[p] = true
					push_warning("[MCP] Port %d rejected by server (project mismatch), won't retry" % p)
				if not _rejected_ports.get(p, false):
					_peers[p] = null
					_timers[p] = 0.0
				else:
					_peers[p] = null  # still clear peer, timer stays 0 to block reconnect
				_last_activity[p] = 0.0
				_ping_timers[p] = 0.0
...
func _send_handshake(p: int) -> void:
	var project_path := ProjectSettings.globalize_path("res://").replace("\\", "/").rstrip("/").to_lower()
